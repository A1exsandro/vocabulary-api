...
async def generate_text(payload: TextGenerateRequest, db: AsyncSession = Depends(get_db)):
...
async def create_manual_text(payload: TextManualCreate, db: AsyncSession = Depends(get_db)):
...
async def update_text(text_id: UUID, payload: TextUpdate, db: AsyncSession = Depends(get_db)):
...
async def delete_text(text_id: UUID, payload: TextDelete, db: AsyncSession = Depends(get_db)):
